...
#[doc = "Field `NSSP` reader - NSS pulse management This bit is used in master mode only. it allows the SPI to generate an NSS pulse between two consecutive data when doing continuous transfers. In the case of a single data transfer, it forces the NSS pin high level after the transfer. It has no meaning if CPHA = â\u{80}\u{99}1â\u{80}\u{99}, or FRF = â\u{80}\u{99}1â\u{80}\u{99}. Note: 1. This bit must be written only when the SPI is disabled (SPE=0). 2. This bit is not used in I2S mode and SPI TI mode."]
...
#[doc = "Field `DS` reader - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the â\u{80}\u{9c}Not usedâ\u{80}\u{9d} values, they are forced to the value â\u{80}\u{9c}0111â\u{80}\u{9d} (8-bit) Note: These bits are not used in I2S mode."]
pub type DS_R = crate::FieldReader<u8, DS_A>;
#[doc = "Data size These bits configure the data length for SPI transfers. If software attempts to write one of the â\u{80}\u{9c}Not usedâ\u{80}\u{9d} values, they are forced to the value â\u{80}\u{9c}0111â\u{80}\u{9d} (8-bit) Note: These bits are not used in I2S mode.\n\nValue on reset: 7"]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[repr(u8)]
pub enum DS_A {
    #[doc = "3: 4-bit"]
...
#[doc = "Field `DS` writer - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the â\u{80}\u{9c}Not usedâ\u{80}\u{9d} values, they are forced to the value â\u{80}\u{9c}0111â\u{80}\u{9d} (8-bit) Note: These bits are not used in I2S mode."]
...
impl R {
    ...
    #[doc = "Bits 8:11 - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the â\u{80}\u{9c}Not usedâ\u{80}\u{9d} values, they are forced to the value â\u{80}\u{9c}0111â\u{80}\u{9d} (8-bit) Note: These bits are not used in I2S mode."]
    #[inline(always)]
    pub fn ds(&self) -> DS_R {
