...
#[doc = "Field `UIFCPY` reader - if IUFREMAP=0 than CNT with read write access else UIFCPY with read only access"]
pub type UIFCPY_R = crate::BitReader<bool>;
#[doc = "Field `UIFCPY` writer - if IUFREMAP=0 than CNT with read write access else UIFCPY with read only access"]
